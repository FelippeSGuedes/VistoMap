...
/**
 * Wrapper em tela cheia pro fluxo guiado de captura da Instalação — mesmo
 * tratamento visual "câmera imersiva" do CaptureCameraModal da vistoria
 * (fundo escuro, cantos de viewfinder), reconstruído do zero: não importa
 * nada de src/components/vistorias.
 */

import { AnimatePresence, motion } from "framer-motion";
import { useEffect } from "react";
import { Camera, X } from "lucide-react";
import { InstalacaoGuidedCaptureFlow } from "./InstalacaoGuidedCaptureFlow";
import type { InstalacaoCaptureBundle } from "@/types";

interface InstalacaoCameraModalProps {
  open: boolean;
...
export function InstalacaoCameraModal({ open, bundle, onChange, onClose, equipmentName }: InstalacaoCameraModalProps) {
  useEffect(() => {
    if (!open) return;
          ...
          >
            <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-white/8 bg-[#05101A]/80 px-4 py-3 backdrop-blur-xl pt-[max(env(safe-area-inset-top),12px)] md:rounded-t-3xl">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-emerald">Câmera guiada</p>
                <h2 className="truncate text-[15px] font-semibold tracking-tight text-white">
                  {equipmentName ?? "Registro fotográfico da instalação"}
                </h2>
              </div>
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain bg-[#0A1A24]">
              <div className="mx-auto w-full max-w-xl px-4 py-4 pb-8">
                <div className="rounded-3xl bg-white p-4 shadow-elev">
                  <InstalacaoGuidedCaptureFlow bundle={bundle} onChange={onChange} onComplete={onClose} />
                </div>
              </div>
            </div>
            ...
            <footer className="sticky bottom-0 z-20 border-t border-white/8 bg-[#05101A]/85 px-4 pb-[max(env(safe-area-inset-bottom),12px)] pt-3 backdrop-blur-xl">
              <div className="mx-auto flex w-full max-w-xl items-center justify-between gap-3">
                <span className="text-xs text-white/70">Concluir as 7 fotos para retornar ao formulário.</span>
                <button
                  type="button"
                  onClick={onClose}
